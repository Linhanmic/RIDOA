#include "cuda/parameter_cuda.cuh"
...
// CUDA核函数：执行参数空间投影
__global__ void parameterSpaceProjectionKernel(
    ...
    double precision)
{
    ...
    {
        // 分配CUDA内存
        CHECK_CUDA_ERROR(cudaMalloc(&d_peakAngles, numPeaks * sizeof(double)));
        CHECK_CUDA_ERROR(cudaMalloc(&d_timepoints, numPeaks * sizeof(double)));
        CHECK_CUDA_ERROR(cudaMalloc(&d_accumulator, elevationSize * azimuthSize * sizeof(double)));

        // 复制数据到GPU
        CHECK_CUDA_ERROR(cudaMemcpy(d_peakAngles, peakAngles.data(),
                                    numPeaks * sizeof(double), cudaMemcpyHostToDevice));
        CHECK_CUDA_ERROR(cudaMemcpy(d_timepoints, timepoints.data(),
                                    numPeaks * sizeof(double), cudaMemcpyHostToDevice));
        CHECK_CUDA_ERROR(cudaMemset(d_accumulator, 0,
        ...
        parameterSpaceProjectionKernel<<<numBlocks, blockSize>>>(
        ...
        // 复制结果回主机
        CHECK_CUDA_ERROR(cudaMemcpy(hostAccumulator.data(), d_accumulator,
                                    elevationSize * azimuthSize * sizeof(double), cudaMemcpyDeviceToHost));
